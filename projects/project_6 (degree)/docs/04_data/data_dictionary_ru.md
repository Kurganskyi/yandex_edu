...
| **Комната** | Помещения различного типа | Код комнаты<br>+ Название комнаты |  |  |
...
| **Устройство** | Устройства пользователя добавленные в приложение умного дома | ID устройства<br>+ Название устройства<br>+ Статус устройства<br>+ Статус режима энергосбережения<br>+ Цвет<br>+ Яркость |  |  |
...
| Цвет | Оттенок цвета устройства (если поддерживается типом устройства) | Символьно-числовое значение | - | Определяется типом устройства в формате JSON |
| Яркость | Интенсивность цвета устройства (если поддерживается типом устройства) | Числовое значение | - | Определяется типом устройства в формате JSON. Диапазон зависит от типа устройства |
...
| Код типа устройства | Уникальный код типа устройства | Символьное значение | - | Значения: "bulb" (лампочка), "socket" (розетка) |
| Тип устройства | Информация о типе устройства (лампочка, розетка) | Символьное значение | - | "bulb" (лампочка), "socket" (розетка) |
| **Сценарий** | Сценарии заданные пользователем для автоматизации управления умным домом | Код сценария<br>+ Название сценария<br>+ Тип запуска<br>+ Время запуска<br>+ Дни недели<br>+ Статус сценария |  |  |
...
| **День недели в сценарии** | Связывает сценарии с днями недели | Код сценария<br>+ Код дня недели<br>+ Время начала<br>+ Время окончания |  |  |
