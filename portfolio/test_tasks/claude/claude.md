...
flowchart TD
    ...
    B --> C{Пауза завершена?}
    C -- Нет --> B
    C -- Да --> D{Тема чувствительная?}

    D -- Да --> O[Передать оператору]
    D -- Нет --> E[Генерация AI]

    E --> F{Дольше TYPING timeout?}
    F -- Да --> G[Заглушка клиенту]
    F -- Нет --> J{Превышен MAX timeout?}
    G --> J
    J -- Да --> O
    J -- Нет --> H{Уверенность >= порога?}

    H -- Да --> I[Ответ клиенту]
    H -- Нет --> O

    I --> K{Новое сообщение или<br/>вызов оператора?}
    K -- Новое сообщение --> B
    K -- Позвать оператора --> P[Оператор ведет диалог]

    O --> L{Оператор в линии?}
    L -- Да --> P
    L -- Нет --> M[Автоответ и очередь]

    E -- Оператор взял диалог --> P
```
