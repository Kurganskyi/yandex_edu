...
**Дата:** 2026-04-08 (rev. 2)
**Автор:** Курганский Антон
...
**Ключевое решение:** AI-оператор — первая линия обработки входящих сообщений. Он классифицирует обращение, генерирует ответ при достаточной уверенности и блокируется (mutex на уровне диалога) при подключении живого оператора. Оператор видит полный контекст работы AI.
...
- Блокировка AI при подключении оператора (mutex)
...
| FR-13 | `[MUST MVP]`       | При событии "оператор взял диалог" —**отменить in-flight генерацию**, запретить отправку уже сформированного буфера. Диалог блокируется для AI (mutex на уровне диалога).                                                                            |
| FR-14 | `[MUST MVP]`       | При эскалации в нерабочее время (нет операторов в линии) — клиент получает**автоответ** с временем следующего ответа; диалог сохраняется в очереди. *(Конкретная формулировка — `[HYPOTHESIS]`, согласуется с PM.)* |
...
| NFR-04 | `[HYPOTHESIS]`     | Архитектура позволяет подключить CRM в v2 без переписывания ядра.                               |
...
7. Если **оператор взял диалог** — немедленный mutex: in-flight генерация отменяется, AI блокируется.
...
### 3.2 Схема состояний диалога (Mermaid)

```mermaid
stateDiagram-v2
    [*] --> DEBOUNCING : новое сообщение

    DEBOUNCING --> DEBOUNCING : новое сообщение в паузе
    DEBOUNCING --> CLASSIFYING : пауза ≥ DEBOUNCE_TIMEOUT

    CLASSIFYING --> OPERATOR_PENDING : чувствительная тема
    CLASSIFYING --> AI_GENERATING : обычный / нераспознанный

    AI_GENERATING --> AI_WAITING : генерация > T сек (заглушка)
    AI_GENERATING --> OPERATOR_PENDING : генерация > MAX_T ИЛИ уверенность < порога
    AI_GENERATING --> AI_ACTIVE : уверенность ≥ порога → ответ отправлен
    AI_GENERATING --> OPERATOR_ACTIVE : оператор взял диалог [mutex]

    AI_WAITING --> OPERATOR_PENDING : генерация > MAX_T
    AI_WAITING --> AI_ACTIVE : уверенность ≥ порога → ответ отправлен
    AI_WAITING --> OPERATOR_ACTIVE : оператор взял диалог [mutex]

    AI_ACTIVE --> DEBOUNCING : новое сообщение клиента
    AI_ACTIVE --> OPERATOR_ACTIVE : клиент нажал "Позвать оператора"
    AI_ACTIVE --> OPERATOR_ACTIVE : оператор взял диалог

    OPERATOR_PENDING --> OPERATOR_ACTIVE : оператор взял диалог
    OPERATOR_PENDING --> OPERATOR_PENDING : нет операторов → автоответ клиенту

    OPERATOR_ACTIVE --> [*] : диалог закрыт
```
...
| `AI_GENERATING`     | `AI_ACTIVE`        | уверенность ≥ порога, ответ отправлен                                     |
| `AI_GENERATING`     | `OPERATOR_ACTIVE`  | оператор взял диалог (mutex → генерация отменена)                     |
| `AI_WAITING`        | `OPERATOR_PENDING` | генерация > MAX_GENERATION_TIMEOUT                                                               |
| `AI_WAITING`        | `AI_ACTIVE`        | уверенность ≥ порога, ответ отправлен                                     |
| `AI_WAITING`        | `OPERATOR_ACTIVE`  | оператор взял диалог (mutex → генерация отменена)                     |
| `AI_ACTIVE`         | `DEBOUNCING`       | новое сообщение клиента                                                              |
...
| ----- | ------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Q1  | Порог уверенности         | Какое минимальное значение достаточно? Кто устанавливает и пересматривает?                                             | `[HYPOTHESIS]` 0.75; пересмотр по результатам пилота                                                                                                                          |
| Q2  | Чувствительные темы     | Кто ведёт список? Как добавляется новая тема?                                                                                                      | —                                                                                                                                                                                                        |
| Q3  | Таймауты                          | Значения TYPING_INDICATOR_TIMEOUT и MAX_GENERATION_TIMEOUT?                                                                                                                    | `[HYPOTHESIS]` 5 сек и 15 сек                                                                                                                                                                      |
| Q4  | Дебаунс                            | Через сколько секунд считать паузу? Максимум сообщений в очереди?                                                                | `[HYPOTHESIS]` 2–3 сек                                                                                                                                                                                |
| Q5  | "Взял диалог"                   | Оператор открыл вкладку / нажал "Принять" / отправил первое сообщение?                                                          | —                                                                                                                                                                                                        |
| Q6  | Механизм недовольства | Кнопка оценки? Триггерная фраза? Повторный вопрос?                                                                                            | `[HYPOTHESIS]` Считать "недовольством" негативный сентимент в следующем сообщении ИЛИ нажатие 👎 — прошу подтвердить |
| Q7  | Нерабочее время             | Что получает клиент при эскалации ночью / в выходные? Автоответ? Только AI без эскалации?                         | `[HYPOTHESIS]` Автоответ "ответим в рабочее время" + диалог в очередь                                                                                          |
| Q8  | Оператор и UI                    | Нужна ли подсказка "Последний ответ дал AI"? Как оператор продолжает без повторения сказанного ботом? | —                                                                                                                                                                                                        |
| Q9  | SLA оператора                    | Есть ли максимальное время ответа после получения диалога? Что при превышении?                                        | —                                                                                                                                                                                                        |
| Q10 | Аналитика                        | Какие метрики с первого дня?                                                                                                                                     | % передач, P95 времени, accuracy                                                                                                                                                            |
...
| Mutex: блокировка AI при подключении оператора (FR-13)                    | `[MUST MVP]`           |
...
| Настройка порогов через UI                                                            | `[HYPOTHESIS]` → v1.1 |

**Out of scope (MVP):** интеграция с CRM, мультиканальность, автозакрытие диалога, дообучение модели, A/B-тестирование порогов, аналитический дашборд.
...
| ---------------------------- | -------------------------------------------------- | ------------------------------------------ |
| `DEBOUNCE_TIMEOUT`         | Пауза перед обработкой       | 2–3 сек`[HYPOTHESIS]`                |
| `CONFIDENCE_THRESHOLD`     | Минимальная уверенность AI | 0.75`[HYPOTHESIS]`                       |
| `TYPING_INDICATOR_TIMEOUT` | Таймаут до заглушки             | 5 сек`[HYPOTHESIS]`                   |
| `MAX_GENERATION_TIMEOUT`   | Максимум генерации              | 15 сек`[HYPOTHESIS]`                  |
...
| AC-06                         | Оператор нажал "Принять" или отправил первое сообщение              | AI немедленно прекращает генерацию (mutex); новые ответы AI в этом диалоге не отправляются   |
...
| FR-06, FR-13        | AC-06, AC-12 | Mutex: оператор взял → AI заблокирован                                |
...
| Q2 | Значения таймаутов              | Не согласованы; влияют на UX                                                                                                                                                                                      | PM + Tech Lead                                |
| Q3 | "Взял диалог"                          | Не определено событие-триггер mutex                                                                                                                                                                          | PM + Frontend                                 |
...
| R3 | Race condition                                   | Закрыт FR-13 (mutex); требует ревью Tech Lead                                                                                                                                                                        | Tech Lead                                     |

### Изменения в рабочем процессе оператора (Change Management)
...
- Как оператор понимает, что диалог уже обработан AI (явная метка в UI)?
...
| ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| **MVP**  | Классификатор тем, генерация по базе знаний, порог уверенности, передача оператору, mutex       | Базовая автоматизация; замер реального снижения нагрузки        |
| **v1.1** | Кнопка оценки ответа AI клиентом, базовый аналитический дашборд, настройка порогов через UI | Измерение качества; инструмент обратной связи для операторов |
